...
export default function EditCommunityModal() {
  const { theme, setShowEditCommunity, pushAction, activeCommunity, communityMap } = useTwirvo();
  
  const currentComm = activeCommunity ? communityMap[activeCommunity] : null;

  const [name, setName] = useState(currentComm?.name || '');
  const [pfp, setPfp] = useState(currentComm?.pfp || '');
  const [banner, setBanner] = useState(currentComm?.banner || '');
  const [bio, setBio] = useState(currentComm?.bio || '');
  const [token, setToken] = useState(currentComm?.token || '');
  const [links, setLinks] = useState<string[]>(['', '', '', '', '', '']);

  const hasInitialized = useRef(false);

  useEffect(() => {
    if (currentComm && !hasInitialized.current) {
      setName(currentComm.name || '');
      setPfp(currentComm.pfp || '');
      setBanner(currentComm.banner || '');
      setBio(currentComm.bio || '');
      setToken(currentComm.token || '');
      
      if (currentComm.links) {
        const initLinks = [...currentComm.links];
        while (initLinks.length < 6) initLinks.push('');
        setLinks(initLinks.slice(0, 6));
      }
  ...
  const formatLink = (url: string) => {
    if (!url) return '';
    let clean = url.trim();
    // Fix 4: Support absolute simulated paths
  ...
  const hasChanged = 
    name !== (currentComm?.name || '') ||
    pfp !== (currentComm?.pfp || '') ||
    banner !== (currentComm?.banner || '') ||
    bio !== (currentComm?.bio || '') ||
    token !== (currentComm?.token || '') ||
    JSON.stringify(links.filter(l => l.trim() !== '')) !== JSON.stringify(currentComm?.links || []);

  const canSubmit = isNameValid && isPfpValid && isBannerValid && isBioValid && isTokenValid && isLinksValid && hasChanged;
  ...
  const handleEditCommunity = () => {
    ...
    pushAction('edit_community', `Edited Community`, activeCommunity, {
      community_name: name,
  ...
  return (
    <div className={`fixed inset-0 ${theme === 'dark' ? 'bg-black/95' : 'bg-white/95'} backdrop-blur-2xl z-[100] flex items-center justify-center p-8 animate-in fade-in`}>
      <div className={`${theme === 'dark' ? 'bg-gray-900 border-gray-800 text-white' : 'bg-white border-gray-200 text-gray-900'} border-4 p-12 rounded-[70px] max-w-2xl w-full max-h-[90vh] overflow-y-auto custom-scrollbar relative shadow-3xl`}>
        <button onClick={() => setShowEditCommunity(false)} className="absolute top-12 right-12 text-gray-500 text-4xl hover:text-blue-500 transition-colors z-20">✕</button>
        
        <div className="mb-12">
            <h2 className="text-5xl font-black uppercase italic text-blue-500 mb-2 tracking-widest leading-none">Edit Community</h2>
            {/* FIX 1: Fee Description */}
            <p className="text-xs font-black uppercase tracking-[0.2em] text-gray-500">
                Updating community metadata on the ledger requires a protocol fee of <span className="text-blue-500">0.1 SOL</span>.
            </p>
        </div>
        ...
        <div className="space-y-8">
          ...
          >
            ...
            {hasChanged ? 'Edit Community (0.1 SOL)' : 'No Changes Detected'}
          </button>
        </div>
      </div>
    </div>
